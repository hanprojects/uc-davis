...
overlayGraphs <- function(subsetX, estMethod, graph, tuning) {
   ...
   if (estMethod == "density") {
      ...
      plot(currentGraph$x, currentGraph$y, col = 1, lwd = 2,
           type = "l", xlim = xlim, ylim = ylim, main="Superimposed Density Plot",xlab="x-axis",ylab="y-axis")
   ...
   # HISTOGRAM OVERLAY
      ...
      plot(currentGraph, col = rgb(173,216,230,max = 255, alpha = 80,
           names = "lt.blue"), xlim = xlim, ylim = ylim, main="Superimposed Histogram",xlab="x-axis",ylab="y-axis")
...
runAnimation <- function(x, estMethod) {
   ...
   if (estMethod == 'hist') {
      ...
      plt <- ggplot(data=df, aes(x=xs, y=ys)) + geom_bar(stat="identity") + transition_states(ds, transition_length = 1, state_length = 1, wrap = FALSE) + labs(title = 'Bins: {closest_state}') + xlab("X Values") + ylab("Y Values")
      print(plt)
   }
   else {
      ...
      plt <- ggplot(m, aes(x = xs, y = ys)) + geom_line() + transition_states(ds, transition_length = 1, state_length = 1, wrap = FALSE) + labs(title = 'Changing Bandwidth') + xlab("X Values") + ylab("Y Values")
      print(plt)
   }
}
...
exploreShape <- function(x,estMethod,tuning,twoAtATime) {
   ...
   while (TRUE) {
      ...
      if (estMethod == 'hist') {
         ...
         plot(graph, main="Histogram of Current Numeric Vector", xlab="x-axis",ylab="y-axis")
      }
      else {
         graph <<- density(subsetX, bw = tuning)
         plot(graph, main="Density Plot of Current Numeric Vector",xlab="x-axis",ylab="y-axis")
      }
